...
<div class="row">
    <div class="col-lg-12 col-md-12 col-md-12">
        <div class="card custom-card">
            ...
            <form id="form" action="{{ route('masyarakat.update', $masyarakat->id) }}" method="POST" enctype="multipart/form-data">
            <div class="card-body">
                @csrf
                @method('PUT')
                {{-- @dd($masyarakat->name) --}}
                <input id="id" name="id" type="hidden" value="{{ $masyarakat->id }}">
                <div class="mb-3">
                    <label for="form-password" class="form-label fs-14 text-dark">Name</label>
                    <input type="text" class="form-control" name="name" value="{{ old('name') ?? $masyarakat->name }}" required>
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="phone_number" value="{{ old('phone_number') ?? $masyarakat->phone_number }}" required>
                    @error('phone_number')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="address" value="{{ old('address') ?? $masyarakat->address }}">
                    @error('address')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
