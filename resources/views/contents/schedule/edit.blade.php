...
<div class="row">
    <div class="col-lg-12 col-md-12 col-md-12">
        <div class="card custom-card">
            ...
            <form id="form" action="{{ route('schedule.update', $schedule->id) }}" method="POST" enctype="multipart/form-data">
                <div class="card-body">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        <select class="form-select" data-trigger name="date" aria-label="Default select example mb-4" required>
                            ...
                            @foreach($availableDates as $availableDate)
                                <option value="{{ $availableDate->id }}" {{ $schedule->date_id == $availableDate->id ? 'selected' : '' }}>{{ $availableDate->tanggal }}</option>
                            @endforeach
                        </select>   
                    </div>
                    <div class="input-group mt-4">
                        <div class="input-group-text text-muted"> <i class="ri-time-line"></i> </div>
                        <input type="text" class="form-control" name="time" id="timepickr1" placeholder="Choose time in 24hr format" value="{{ $schedule->time }}" required>
                    </div>             
                </div>
